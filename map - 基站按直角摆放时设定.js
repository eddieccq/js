...
var zoomdefault = 19;	//默认放大比例
var zoommax=22;
var zoommin=17;
var pxsize = 821.1;		//1像素代表 1CM
var pysize = 543.3;
...
var readcard_p0=[0,0];		//读卡器14437
var readcard_p1=[0,pysize];	//读卡器14438
var readcard_p2=[pxsize,0];	//读卡器14439
var precision = 10;			//容差精度，设计逻辑为，新精度值若超出旧精度允许的容差，则显示，否则不更新状态。
var debugMode = true;		//是否调试模式，如果是，则持续更新状态，不是，则按容差来处理。
//实时轨迹显示
var maxHis=100;		//最大轨迹显示个数
...
function initmap(jobid) {
	...
	loadDetector();// 加载探测器信息
	 ...
	 //  var obj1 = { FLAGCONFIRM : "0", IDNO : "4258", PERSONNAME : "杜青青",
	 //  alarm_status : "0", areadescription : "非生产区", cardno : "3", flag_work :
	 //  "1", helmetflag : "1", id : "1128370", organname : "陡电项目部", readerno :
	 //  "1", relationid : "1128370", tid : "1002159", timein : "2016-01-21",
	 //  timeread : "2017-03-28", x : "-1.48695209558051", y :
	 //  "0" };
	  
	 // var g1 = gethelmet(obj1); 
	 // helmetsLayer.getSource().addFeature(g1);
	 

	/*
	 * // 下面把上面的图标附加到地图上，需要一个动画锚点 ol.Overlay var anchor = new ol.Overlay({
	 * element: document.getElementById('anchor') }); //关键的一点，需要设置附加到地图上的位置
	 * anchor.setPosition(center); // 然后添加到map上 map.addOverlay(anchor);
	 */
...
function refreshhelmets(arr) {
	...
	for (var i = 0; i < arr.length - 1; i++) {
		...
		if(obj!=null){
			...
			console.log("当前点："+ getRectCoordinates(point) + 
				" 距基站337: " + getLengthByTwoPoints(point,readcard_p0) +
				" 338: " + getLengthByTwoPoints(point,readcard_p1) +
				" 339: " + getLengthByTwoPoints(point,readcard_p2));
		}
